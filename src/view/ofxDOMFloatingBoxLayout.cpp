#include "ofxDOMFloatingBoxLayout.h"


ofxDOMFloatingBoxLayout::ofxDOMFloatingBoxLayout(DOM::Element* parent, DOM::Orientation orientation):
	ofxDOMBoxLayout(parent, orientation){
}

ofxDOMFloatingBoxLayout::~ofxDOMFloatingBoxLayout(){
}

void ofxDOMFloatingBoxLayout::doLayout(){
		...
		for (DOM::Element* element: children()){
			...
			if (element){
				...
				if(elementFloating(element)){
					...
					DOM::LayoutFloat layoutfloat = element->getAttribute<DOM::LayoutFloat>("float");

					switch(layoutfloat){

						case DOM::LayoutFloat::LEFT: {
...
bool ofxDOMFloatingBoxLayout::elementFloating(DOM::Element* e){
	if(!e->isHidden()){
		if(e->hasAttribute("position")){
			if(e->getAttribute<DOM::LayoutPosition>("position") == DOM::LayoutPosition::STATIC){
...
void ofxDOMFloatingBoxLayout::setNextLeftFloatPosition(ofPoint p, DOM::Element* e){
...
ofPoint ofxDOMFloatingBoxLayout::nextLeftFloatPositionLoop(DOM::Element* e, DOM::Element* s, const ofPoint& pos){

	ofPoint res = pos;

	for (DOM::Element* sibling: children()){
		...
		if(sibling){
			...
			if(elementFloating(sibling)){
				...
				DOM::LayoutFloat lf = sibling->getAttribute<DOM::LayoutFloat>("float");
...
void ofxDOMFloatingBoxLayout::setNextRightFloatPosition(ofPoint p, DOM::Element* e){
...
ofPoint ofxDOMFloatingBoxLayout::nextRightFloatPositionLoop(DOM::Element* e, DOM::Element* s, const ofPoint& pos){
...
float ofxDOMFloatingBoxLayout::getRight(DOM::Element* e){
...
float ofxDOMFloatingBoxLayout::getLeft(DOM::Element* e){
...
float ofxDOMFloatingBoxLayout::getTop(DOM::Element* e){
...
float ofxDOMFloatingBoxLayout::getBottom(DOM::Element* e){
...
float ofxDOMFloatingBoxLayout::getWidth(DOM::Element* e){
...
float ofxDOMFloatingBoxLayout::getHeight(DOM::Element* e){
...
void ofxDOMFloatingBoxLayout::setPosition(DOM::Element* e, ofPoint p){
	if(e->hasAttribute("margin-left")){
